				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Lean Lib")
							if fold then
							else
								makefolder("./Lean Lib")
							end
							fold = isfolder("./Lean Lib/Themes")
							if fold then
							else
								makefolder("./Lean Lib/Themes")
							end
							fold = isfolder("./Lean Lib/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Lean Lib Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Lean Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Lean Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Lean Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Lean Lib" then
		library.WorkspaceName = convertfilename(windowName, "Lean Lib")
	end
	local LeanLibrary = Instance_new("ScreenGui")
	library.MainScreenGui, MainScreenGui = LeanLibrary, LeanLibrary
	...
	LeanLibrary.Name = "     "
	LeanLibrary.Parent = library.gui_parent
	LeanLibrary.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	LeanLibrary.DisplayOrder = 10
	LeanLibrary.ResetOnSpawn = false
	main.Name = "main"
	main.Parent = LeanLibrary
	...
	function windowFunctions:CreateTab(options, ...)
		options = (options and (type(options) == "string") and resolvevararg("Tab", options, ...)) or options or {
			Name = "Lean Style: Elite Lego Hax"
		}
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Lean Lib") then
						makefolder("./Lean Lib")
					end
					local common_string = "./Lean Lib/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Lean Lib/" .. tostring(custom_workspace or library.WorkspaceName)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://11262387451",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "rbxassetid://11262387451",
